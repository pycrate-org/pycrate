    ...
    def _resolve_ref(self, attr):
        ...
        while True:
            if par is None:
                ...
                logger.warning('%s, unable to resolve reference: do it by hand within the Python file generated' % ref[0])
    ...
    def _parse_arithm(cls, text):
        ...
        if err:
            ...
            while text and op_clo[1] < op_clo[0]:
                ...
                toks.append(text[0:1])
                text = text[1:]
            logger.warning('unable to process arithmetic expression, %s: '\
                           'do it by hand within the Python file generated' % ''.join(toks))
            return ('# unprocessed: %s' % ''.join(toks), (None, None)), text.lstrip()
        #
        ...
        try:
            ...
            logger.error('unable to process arithmetic expression, (%s)' % ''.join([str(t[1]) for t in toks]))
            # it's preferrable to break if we reach here
            assert()
            return ('# unprocessed: %s' % ''.join([str(t[1]) for t in toks]), (None, None)), text.lstrip()
...
def translate_text(text=u'', **kwargs):
    ...
    while ObjList:
        ...
        if len(ObjList) == obj_len:
            logger.warning('unable to define objects, %r' % ObjList)
            break
    #
...
def process_definition(text, Objs, **kwargs):
    ...
    m = re.search('::=', text)
    if not m:
        logger.info('CSN.1 object assignment sign ::= not found')
        return ''
    # get the object name
